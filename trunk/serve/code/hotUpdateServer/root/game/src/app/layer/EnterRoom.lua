...
local EnterRoom = class("EnterRoom",require("app.layer.BaseLayer"))

function EnterRoom:ctor()
	EnterRoom.super.ctor(self)
...
function EnterRoom:onTouchEnded(touch,event)
		...
		if btn:hitTest(touch:getLocation(),false) then
			...
			elseif tag == "del" then
				if string.len(self.roomid) > 0 then
					self.nums[string.len(self.roomid)]:setString("")
				...
				if string.len(self.roomid) == 6 then
					if self.isjoin == false then
						self.isjoin = true
						EventMgr.doAction(1004,{id = self.roomid})
					end
				end
			end
		end
...
function EnterRoom:reset()
	self.roomid = ""
	for i=1,#self.nums do 
		self.nums[i]:setString("")
	end
end

function EnterRoom:enterRoomFail(data)
	local callback = function()
		self.isjoin = false
		self:reset()
	end
	MessageBox.show(data["msg"],{button = {{name = "OK",callback = callback}}})
end
return EnterRoom