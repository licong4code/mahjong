...
function HomeLayer:loginSuccess(data)
	...
	elseif status == 1 then --等待]
		...
		require("app.layer.GameLayer").new(data):addTo(self,10)
...
-- function HomeLayer:enterRoomSuccess(data)
-- 	for k,v in pairs(self.buttons) do 
-- 		v:setVisible(false)
-- 	end
...
-- 		globalUserData:init(room[globalUserData["id"]])
...
function HomeLayer:connectSuccess()
	EventMgr.doAction(1001,{uid = Util.getUID()})
...
function HomeLayer:onUserEnter(data)
	...
	if data["status"] == 2 then
		...
		globalUserData:initFromRoom(room)
		require("app.layer.GameLayer").new(data):addTo(self,1)
	end
