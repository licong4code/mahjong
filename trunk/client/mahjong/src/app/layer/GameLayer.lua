...
function GameLayer:onDissolveSuccess()
	...
	for id,user in pairs(self.users) do 
		...
		if id == globalUserData["id"] then
			...
			for i=1,#touchdata do
				...
				local touch = touchdata[i]
				local touchtype = touch["type"] 
				if touchtype == 0 then --吃
				elseif touchtype == 1 then --碰
					for j = 1,3 do 
						local sp = getTouchCardView(touch["value"]):addTo(self)
						sp:pos(beginx + sp:getContentSize().width*j,90)
					end
				elseif touchtype == 2 then --暗杠
					for j = 1,3 do 
						local sp = getTouchCardView(touch["value"]):addTo(self)
						sp:pos(beginx + sp:getContentSize().width*j,90)
					end
					local sp = getTouchCardView(touch["value"]):addTo(self)
					sp:pos(beginx + sp:getContentSize().width*2,120)
				elseif touchtype == 3 then --捡杠
					for j = 1,3 do 
						local sp = getTouchCardView(touch["value"]):addTo(self)
						sp:pos(beginx + sp:getContentSize().width*j,90)
					end
...
function GameLayer:onUserPeng(data)
	...
	self.wait = 0
	dump(globalUserData)
	if globalUserData["group"] then
		require("app.layer.ChooseLayer").new(globalUserData["group"]):addTo(self)
	end
end
...
function GameLayer:onUserHu(data)
	dump(data,"hu")
	require("app.layer.ResultLayer").new(data):addTo(self)
end
return GameLayer
-- 