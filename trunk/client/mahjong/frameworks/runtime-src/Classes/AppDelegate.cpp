...
bool AppDelegate::applicationDidFinishLaunching()
{
    ...
    if(!glview) {
        string title = "mahjong";
        glview = cocos2d::GLViewImpl::create(title.c_str());
        director->setOpenGLView(glview);
        director->startAnimation();
    }
	glview->setDesignResolutionSize(1290, 720, ResolutionPolicy::SHOW_ALL);
    ...
    FileUtils::getInstance()->addSearchPath(FileUtils::getInstance()->getWritablePath() + "update/");
    FileUtils::getInstance()->addSearchPath("res/");
#if 1
    // use luajit bytecode package
    stack->setXXTEAKeyAndSign("8c1f4c6e", "lcandx10");
    
#ifdef CC_TARGET_OS_IPHONE
    if (sizeof(long) == 4) {
        stack->loadChunksFromZIP("framework32.zip");
        stack->loadChunksFromZIP("game32.zip");
    } else {
        stack->loadChunksFromZIP("framework64.zip");
        stack->loadChunksFromZIP("game64.zip");
    }
#else
